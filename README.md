...
- ProxyIP 可以按地区挑了，不用再自己找 IP 贴进去
  - 配置页「自定义 ProxyIP」下面多了个搜索框，输中文、英文或机场码都能搜
  - 支持口语说法：搜「美西」出洛杉矶/圣何塞/西雅图，搜「日本」出东京/大阪/福冈/那霸
  - 点一下自动填一个该地区的 IP，重复点换一批
  - 数据 102257 条，覆盖 91 个数据中心 38 个国家，来自 [cfnew-ipdb](https://github.com/byJoey/cfnew-ipdb)
  - 优先给复测通过的 IP；数据由 Worker 侧代拉，浏览器不直连数据源，国内也能用
- 详见「[按地区选 ProxyIP](#按地区选-proxyip)」
...
| `p` | proxyip | 可选，自定义ProxyIP地址和端口，支持 IPv4/IPv6/域名。设置后 `wk` 地区匹配失效（互斥）。也可在节点 path 里单独指定 。v3.0 起可在面板按地区搜着选，见「[按地区选 ProxyIP](#按地区选-proxyip)」 |
...
v3.0 开始提供。以前填 ProxyIP 得自己去别处找一个能用的 IP，现在直接在面板里选。

位置：配置管理 → 自定义 ProxyIP (p) 输入框下方。
...
列表里每条会标该地区有多少个 IP、其中多少个通过了复测。点一下就填进输入框，不满意再点一次换一个。

几点说明：

- 数据来自 [byJoey/cfnew-ipdb](https://github.com/byJoey/cfnew-ipdb)，102257 条 IP / 91 个数据中心 / 38 个国家，全 443 端口
- 优先给复测通过的 IP，不够了才用首轮结果补
- Worker 侧代拉数据并缓存 6 小时，浏览器不直连 GitHub，国内网络也能用
- 填了 ProxyIP 之后 `wk` 地区匹配会自动失效，这是原有的互斥逻辑，没变
- IP 可用性会随时间变化，选完建议用下面的延迟测试验一下

也可以直接拉数据自己用：

```bash
curl -s https://raw.githubusercontent.com/byJoey/cfnew-ipdb/main/colo/HKG.txt
curl -s https://raw.githubusercontent.com/byJoey/cfnew-ipdb/main/index.json
```
